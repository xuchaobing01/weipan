...
$('.confirmBtn').css('backgroundColor','#f04447');
	...
	function send() {
		...
		$.ajax({
			...
			data : 'passwd=' + pass + '&tel=' + phone +  '&invite=' + inviteCode +'&token=' + $("#token").val() + '&reg_xieyi=1&proxy_id=' + $("#proxy_id").val()+"&SMSCode="+SMSCode+"&inviterMobile="+inviterMobile +"&act="+$("#act").val(),
			dataType : 'JSON',
			async : false,
