...
class TextHandler extends MessageHandler {
	...
	public function handle() {
		...
		if(preg_match("/天气/",$key)) {
			return tianqi(explode(' ', $key));
		}
		
		
		
		if(preg_match("/微现场@/",$key)) {
			$key = $this->wxc($key);
		}
	...
	public function wxc($key){
		...
		$d = explode("@",$key);
		//查询商家是否有活动
		$active = M("wxc_active")->where(['token'=>$this->token])->order('id desc')->find();
		if($active['start_time']<time()){
			return $this->text('该活动尚未开始');
		}
		if($active['end_time']>time()){
			return $this->text('该活动已经结束');
		}	
		if(!M('wxc_user')->where(['open_id'=>$this->fromUserName])->find()){
			//获取用户信息
			$user = $util->getUserInfo($from_user);
			...
			$data['num'] = M('wxc_user')->where(['token'=>$this->token])->count()+1;
			M('wxc_user')->add($data);	
		}
		$this->newsImages($active['id']);
	}
	
	public function newsImages($id){
		$img   = M('Img');
		$news     = $img->field('id,text,pic,url,title,is_multi,more_news')->where(['id'=>1011])->find();	
		$news['url'] = $this->U('Wap/Wxc/qiandao',['id' => $id,'wechat_id'=>$this->fromUserName]);	
		$return[] = array(
				$news['title'],
				$news['text'],
				$news['pic'],
		...
		if($news['is_multi'] == 1 ){//多图文
			...
			if($more_news!=false && is_array($more_news)){
				foreach($more_news as $key => $value){
					switch ($key){
						case 0:
							$value['url'] =$this->U('Wap/Wxc/qiandao',['id' => $id,'wechat_id'=>$this->fromUserName]);	
							break;
						case 1:
							$value['url'] =$this->U('Wap/Wxc/qiandao1',['id' => $id,'wechat_id'=>$this->fromUserName]);
							break;
						case 2:
							$value['url'] =$this->U('Wap/Wxc/qiandao2',['id' => $id,'wechat_id'=>$this->fromUserName]);
							break;
