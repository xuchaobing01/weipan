<?php
/**
 *@ class EstateController 微房产模块控制器
 */
namespace User\Controller;
use Spark\Util\Page;
class EstateController extends UserController{
        ...
        if(IS_POST){
			...
			if($es_data == null){
				...
				if($data->where($wh)->save($_POST)){
                    ...
                    $data1['module'] = 'Estate';
                    $data1['token'] = session('token');                    
                    $da['keyword'] = I('post.keyword','','trim');
                    M('Keyword')->where($data1)->save($da);

                    $this->success('修改成功',U('Estate/index'));exit;
                }else{
    ...
    public function son_del(){
        ...
        if($check == null){
            ...
            if($isok){
               $this->success('删除成功',U('Estate/son'));exit;
           }else{
                $this->error('删除失败',U('Estate/son'));exit();
           }
        }
    }
    ...
    public function housetype(){
        ...
        foreach ($son_type as $key => $value) {
			$housetype[$key]['title'] = $value['title'];
        }
		//var_dump($housetype);exit;
        ...
        if(IS_POST){
            ...
            if($check == null){     
				...
				if($t_housetype->add($_POST)){                     
					$this->success('添加成功',U('Estate/housetype'));
					 exit;
				}else{
					$this->error('服务器繁忙,请稍候再试');exit;
				}                 
           }else{            
				$wh = array('token'=>session('token'),'id'=>I('post.id')); 
				if($t_housetype->where($wh)->save($_POST)){
                    $this->success('修改成功',U('Estate/housetype'));
                    exit;
                }else{
                    $this->error('操作失败');exit;
                }
    ...
    public function housetype_del(){
       $housetype = M('Estate_housetype');
        ...
        if($check == null){
            ...
            if($isok){
               $this->success('删除成功',U('Estate/housetype'));exit;
			}else{
                $this->error('删除失败',U('Estate/housetype'));exit;
			}
        }
    }
    ...
    public function album_add(){
        ...
        $t_album = M('Estate_album');
        $poid = I('get.poid',0,'int');
        $check = $t_album->where(array('token'=>session('token'),'poid'=>$poid))->find();
        $this->assign('album',$check);
        if(IS_POST){
            if($check == NULL){  
                $check_ex = $t_album->where(array('token'=>session('token'),'poid'=>I('post.poid')))->find();  
                if($check_ex){
	...
	public function album_del(){
		$id = I('get.id',0,'intval');
		if($id){
			M('Estate_album')->where(['token'=>session('token'),'poid'=>$id])->delete();
		}
		$this->success('删除成功！',U('album'));
	}
    ...
    public function reservation_edit(){
         if(IS_POST){
            $data=D('Reservation');
            $where=array('id'=>I('post.id',0,'intval'),'token'=>session('token'));
    ...
    public function aboutus(){
        ...
        if(IS_POST){   
            if($check == null){
             ...
             $wh = array('id'=>I('post.id',0,'intval')); 
