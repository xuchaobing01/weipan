...
use Spark\Util\Page;
class VoteyController extends UserController{
    public function index(){
		//$where['token'] = session('token');
		$where=array('token'=>session('token'),'type'=>1);
        $list=M('Votex')->where($where)->order('id DESC')->select();
        $count = M('Votex')->where($where)->count();
    ...
    public function del_record(){
        ...
        if(M('vote_record')->where(array('id'=>$id))->delete()){
            ...
            $this->success('删除成功',U('Votey/index',array('token'=>session('token'))));
        }
    }
    ...
    public function add(){
        if(IS_POST){
            ...
            if($data->create()!=false){
                if($id=$data->add()){
                    $this->success('添加成功',U('Votey/index',array('token'=>session('token'))));
                }
				else{
	...
	public function add_item(){
		...
		if(IS_POST){
            ...
            if($data->create()!=false){
                if($id=$data->add()){
                    $this->success('添加成功',U('Votey/items',array('vote_id'=>$_GET['vote_id'])));
                }
				else{
                    $this->error('服务器繁忙,请稍候再试');
                }
	...
	public function item_edit(){
		...
		if(IS_POST){
            ...
            if($addnum>0){
                ...
                $this->success('票数添加成功',U('Votey/items',array('token'=>session('token'),'vote_id'=>$_GET['vid'])));
            }elseif($data->create()!=false){
                if($id=$data->save()){
                    $this->success('编辑成功',U('Votey/items',array('token'=>session('token'),'vote_id'=>$_GET['vid'])));
                }
				else{
                    $this->error('服务器繁忙,请稍候再试');
                }
	...
	public function item_del(){
        ...
        if($result){
            ...
            $this->success('删除成功',U('Votey/items',array('token'=>session('token'),'vote_id'=>$_GET['vid'])));
        }
		else{
            $this->error('非法操作！');
        }
        ...
        if(IS_POST){
            ...
            if($data->create()){
                if($data->where($where)->save($_POST)){
                    $this->success('修改成功!',U('Votey/index',array('token'=>session('token'))));exit;
                }
				else{
                    $this->success('修改成功',U('Votey/index',array('token'=>session('token'))));exit;
                }
    ...
    public function changetop(){//置顶
    	...
    	if($istop == $_GET['istop']){
    		$this->success('重复操作',U('Votey/items',array('token'=>session('token'),'vote_id'=>$vote_id)));
    	}
    	else{
    		$res = $leave_model->where(array("id"=>$id))->setField("istop",$_GET['istop']);
    		if($res){
    			if($_GET['istop']==1)
    				$this->success("审核成功",U('Votey/items',array('token'=>session('token'),'vote_id'=>$vote_id)));
    				
    			else
    				$this->success("取消审核成功",U('Votey/items',array('token'=>session('token'),'vote_id'=>$vote_id)));
    		}
    		else{
    			$this->error("操作失败",U('Votey/items',array('token'=>session('token'),'vote_id'=>$vote_id)));
    		}
    	}
