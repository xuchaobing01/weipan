...
class TurntableController extends UserController{
	...
	public function sn(){
		...
		$record = M('Lottery_user')->where('lottery_id ='.$id.' and status != 0')->order('id desc')->select();
		...
		if(IS_POST){
			...
			if($data->create()!=false){	
				if($id=$data->add()){
					...
					$this->success('活动创建成功',U('Turntable/index'));
				}
				else{
		...
		if(IS_POST){
			...
			if($data->create()){
				if($data->where($where)->save($_POST)){
					...
					$this->success('修改成功',U('Turntable/index',array('token'=>session('token'))));
				}
				else{
