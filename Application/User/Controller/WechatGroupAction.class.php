...
class WechatGroupController extends UserController{
	...
	public function  class_del(){
		$class=M('Diymen_class')->where(array('token'=>session('token'),'pid'=>I('id')))->order('sort desc')->find();
		//echo M('Diymen_class')->getLastSql();exit;
		if($class==false){
			$back=M('Diymen_class')->where(array('token'=>session('token'),'id'=>I('id')))->delete();
			if($back==true){
				$this->success('删除成功');
			}else{
				$this->error('删除失败');
	...
	public function  class_edit(){
		if(IS_POST){
			$_POST['id']==I('id');
			$this->all_save('Diymen_class','class_edit?id='.I('id'));
		}else{
			$data=M('Diymen_class')->where(array('token'=>session('token'),'id'=>i('id')))->find();
			if($data==false){
