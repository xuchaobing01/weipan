...
use Spark\Util\Page;
class PowerController extends UserController{
	public $token;
	public $wechat_id;	
	...
	public function addactivity (){
		if(IS_POST){
			...
			$_POST['starttime'] = strtotime(I('post.statdate'));
			...
			if($data->create()!=false){
				if($id=$data->add()){
					$this->success('添加成功',U('Power/set',array('token'=>session('token'))));
				}
				else{
		...
		if(IS_POST){			
			...
			if($data->create()){
				if($data->where($where)->save($_POST)){
					$this->success('修改成功!',U('Power/set',array('token'=>session('token'))));exit;
				}
				else{
					$this->success('修改成功',U('Power/set',array('token'=>session('token'))));exit;
				}
	...
	public function activitydel(){
		...
		if(IS_GET){
			...
			if($r==true){
				$this->success('操作成功',U('Power/set'));	
			}
			else{
				$this->error('服务器繁忙,请稍后再试',U('Power/activity'));
			}
		}	
	}
	public function del(){//单项删除
		...
		if($res){
			$this->success("删除成功",U('User/Power/activityimg',array('wecha_id'=>$this->wecha_id,'token'=>$this->token)));
		}else{
			$this->success("删除失败",U('User/Power/activityimg',array('wecha_id'=>$this->wecha_id,'token'=>$this->token)));
		}
	}
	...
	public function addmessage(){
		if(IS_POST){
			...
			if($res){
				$msgarr['id']=$res;
				$this->success('留言成功',U('Power/message',array('id'=>$id)));
			}else{
	
				$this->success('留言失败',U('Power/message',array('id'=>$id)));
			}
		}else{
			$id=$_GET['id'];
			$this->assign('id',$id);
	...
	public function changetop(){//置顶
		...
		if($istop == $_GET['istop']){
			$this->error("重复操作",U('User/Power/message',array('id'=>$cid)));
			...
			if($res){
				if($_GET['istop']==1)
					$this->success("取消置顶成功",U('User/Power/message',array('id'=>$cid)));
				else 
					$this->success("置顶成功",U('User/Power/message',array('id'=>$cid)));
			}
			else{
				$this->error("置顶失败",U('User/Power/message',array('id'=>$cid)));
			}
		}
	...
	public function replyedit(){
		...
		if(IS_POST){
			...
			if($res){
				$this->success("编辑成功",U('User/Power/message',array('id'=>$cid)));
			}else{
				$this->success("编辑失败",U('User/Power/message',array('id'=>$cid)));
			}
		}else{
	...
	public function checkOne(){//单项审核
		...
		if($checked == 1){
			...
			if($res){
				$this->success("审核成功",U('User/Power/message',array('id'=>$cid)));
			}
			else{
				$this->error("审核失败",U('User/Power/message',array('id'=>$cid)));
			}
		}
	}
	public function deled(){//单项删除
		...
		$res = $leave_model->delete($id);
		if($res){
			$this->success("删除成功",U('User/Power/message',array('wecha_id'=>$this->wecha_id,'token'=>$this->token,'id'=>$cid)));
		}else{
			$this->success("删除失败",U('User/Power/message',array('wecha_id'=>$this->wecha_id,'token'=>$this->token,'id'=>$cid)));
		}
	}
	/**
	 * @管理员回复
	...
	public function insert(){//添加回复 插入数据库
		...
		if(in_array("0",$result)){
			$this->error("回复失败",U('User/Power/message',array('token'=>$this->token)));
		}
		else{
			$this->success("回复成功",U('User/Power/message',array('token'=>$this->token)));
		}
	}
	public function replyChked(){//回复内容单项审核
		...
		if($checked == 1){
			$this->success("已审核",U('User/Power/message',array('msgid'=>$msgid)));
		}
		else{
			$res = $reply_model->where(array("id"=>$id))->setField("checked",1);
			if($res){
				$this->success("审核成功",U('User/Power/reply',array('msgid'=>$msgid)));
			}
			else{
				$this->error("审核失败",U('User/Power/reply',array('msgid'=>$msgid)));
			}
		}
	}
