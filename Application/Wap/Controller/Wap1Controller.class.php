...
class Wap1Controller extends Controller {
	protected $allowFromWeb = true;//是否允许非微信浏览器访问
	protected $strict_mode = false;//是否获取访问者openid
    ...
    protected function _initialize() {
		...
		if(!is_valid_openid($this->wechat_id) && $this->strict_mode){//strpos($agent,"MicroMessenger")){
    ...
    protected function is_open_action(){
    	return  in_array(CONTROLLER_NAME, ['Login','Seccode','Wap']);//  && in_array(ACTION_NAME,['login','reg','seccode']);
    }
