	...
	<div class="tabbable-custom">
	  <!-- Nav tabs -->
		<ul class="nav nav-tabs" role="tablist">
			<li role="presentation"><a href="<?php echo U('index');?>" aria-controls="home" role="tab">单图文回复</a></li>
			<li role="presentation" class="active"><a href="#" aria-controls="profile" role="tab">多图文回复</a></li>
		</ul>
	</div>
	<div class="page-header wm-page-header">
		<a class="btn btn-success" href="<?php echo U('Img/multi_img_edit');?>">
		<span class="glyphicon glyphicon-picture"></span>&nbsp;新增图文回复</a>
	</div>
	<table class="table wm-table-bordered table-stripped">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td width="120px;" style="min-width:120px;max-width:200px;"><div class="wrap-text"><?php echo ($vo["text"]); ?></div></td>
				<td><?php if(($vo["type"]) == "1"): ?>完全匹配<?php else: ?>模糊匹配<?php endif; ?></td>
                <td><?php echo ($vo["click"]); ?></td>
                <td><?php echo (date('Y-m-d',$vo['updatetime'])); ?></td>   
				<td class="norightborder">
					<a class="btn btn-xs btn-primary" href="<?php echo U('Img/multi_img_edit',array('id'=>$vo['id']));?>" title="编辑图文自定义回复">编辑</a>
					<a class="btn btn-xs btn-danger" href="<?php echo U('Img/del',array('id'=>$vo['id']));?>">删除</a></td>
