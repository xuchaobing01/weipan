        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">更改用户所属经纪人</h4>
                </div>
                <div class="modal-body">
                    <form id="form1" class="form" method="post" action="<?php echo U('Channel/changeAgent');?>">
                        <input type="hidden" name="id" value="<?php echo ($id); ?>" />
                        <input type="hidden" name="agent_id" value="<?php echo ($agent_ids); ?>" />

                        <div class="form-group">
                            <label for="score">请选择代理商</label>
                                <select  name="phone" id='phone' class="form-control">
                                    <option  value="0">--请选择--</option>
                                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option  value="<?php echo ($list["phone"]); ?>" <?php if($agent_id == $list['phone']): ?>selected="selected"<?php endif; ?> ><?php echo ($list["username"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                        </div>
                        <button type="submit" class="btn btn-primary">确定</button>
                        <a href="<?php echo U('Channel/custom');?>" class="btn btn-primary">返回</a>
                    </form>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
