...
<div class="container-fluid">
	...
	<ul class="nav nav-tabs wm-tabs">
		...
		<li class="active"><a href="javascript:void(0)">会员管理</a></li>
		 ...
		 <li><a href="<?php echo U('Member/adduser');?>">添加会员</a></li>
	...
	<div class="page-header wm-page-header">
	 	...
	 	<form method="get" action="/User/Member/index.html" class="form-inline">
...
<button data-toggle="modal" data-backdrop="static" data-target="#modal"
								class="btn btn-default btn btn-xs btn-danger" type="button" onclick="sub(<?php echo ($list['id']); ?>,<?php echo ($list['star']); ?>)">更改身份</button>

			<a href="<?php echo U('changeChannel?id='.$list['id'],array('channel_id'=>$list['channel_id']));?>" class="btn btn-xs btn-danger">更改代理商</a>
...
<span class="btn btn-xs btn-danger" onclick='spark.confirm_jump("会员冻结后不可登录个人页面，您确定要继续吗?", "<?php echo U('User/Member/del',['id'=>$list['id'],'status'=>1]);?>")'>冻结
			...
			<a class="btn btn-xs btn-danger" onclick='spark.confirm_jump("删除会员会导致此会员不可用，您确定要继续吗?", "<?php echo U('User/Member/delUser',['id'=>$list['id'],'status'=>0]);?>")'>删除</a>
			</TD>
